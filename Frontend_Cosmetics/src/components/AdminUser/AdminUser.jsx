...
const AdminUser = () => {
    ...
    const fetchGetDetailUser = async (rowSelected) => {
        ...
        if (res?.userDetail) {
            setStateUserDetail({
                ...
                isAdmin: res?.userDetail?.isAdmin,
    ...
    const dataTable = users?.users?.length && users?.users?.map((user) => {
    ...
    return (
        <div>
            ...
            <div style={{ marginTop: '20px' }}>
                <TableComponent handleDeleteMany={handleDeleteManyUser} isLoading={isLoading} columns={columns} data={dataTable} onRow={(record, rowIndex) => {
                    return {
                        onClick: event => {
